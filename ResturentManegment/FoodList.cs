...
namespace ResturentManegment
{
    public partial class FoodList : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (IsValid())
            {
                SqlCommand cmd = new SqlCommand("INSERT INTO foodTable VALUES(@foodname,@price)", db.con);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (foodId > 0)
            {
                SqlCommand cmd = new SqlCommand("UPDATE foodTable SET foodname=@foodname,price=@price WHERE foodid=@foodid", db.con);
                cmd.CommandType = CommandType.Text;
                cmd.Parameters.AddWithValue("@foodname", textFoodName.Text);
                cmd.Parameters.AddWithValue("@price", textFoodPrice.Text);
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (foodId > 0)
            {
                ...
                SqlCommand cmd = new SqlCommand("DELETE FROM foodTable WHERE foodid=@foodid", db.con);
                cmd.CommandType = CommandType.Text;
